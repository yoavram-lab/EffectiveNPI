...
\begin{figure}[b!]
    ...
    \label{fig:tau-summary} %TODO maybe to put some footnote about the realibility of the result for Sweden and Denmark?
\end{figure}
...
For all of the 11 European countries the out-of-sample root mean squared error (RMSE) is lower for the model with a free~$\tau$ (\autoref{table:RMSE}). We don't check it for Wuhan, China, because we have no out-of-sample data for it.  % TODO reference? ; add table ; find more data for Wuhan?
When we compare the models using WAIC~(\autoref{eq:WAIC}, \autoref{table:WAIC}), the model with a free~$\tau$ is strongly preferred in all of 12 countries except Sweden and Denmark. Both Sweden and Denmark, don't have one visual peak in daily cases for the dates the models are evaluated, so probably it is not enough data for the free~$\tau$ model.
...
In Wuhan, China, a lockdown was ordered on Jan 23~\citep{Li2020}, but we estimate the effective start of NPIs to be more than a week later, at Feb~3 ($\pm$2.85 days 95\% CI; \autoref{fig:late}). % TODO CHECK
...
Germany... %TODO
Sweden and Denmark show early effective start of NPIs. However, for both of the contries, the dataset that we use doesn't contain dates with the peak of the pandemic. WAIC model comparison method doesn't prefer the model with a free~$\tau$ on the model with a fixed~$\tau$.
Therefore we can't trust in the outcome of the free~$\tau$ model for those countries.
...
We have found that the evidence supports a model in which the parameters change at a specific time point $\tau$ over a model without such a change-point in 10 out of 12 regions (i.e. free or fixed model in \autoref{table:WAIC}). 
