...
We find mostly late, but also early effects of interventions. For example, Italy implemented a nationwide lockdown on Mar~11, but we infer the effective date on Mar~17 ($\pm$2.99 days 95\% CI). In contrast, Germany announced a lockdown on Mar~22, but we infer an effective start date on Mar~19 ($\pm$ 1.05 days 95\% CI).
\hl{
...
\hl{We conclude by demonstrating how differences between the official and effective start of NPIs can confound assessments of their impact.}
...
%%% Fig - tau summary %%%
% generated with /python Fig_tau_summary.py 2020-05-26-Apr11/
\begin{figure}[b!]
    \centering
	...
	Here, $\hat{\tau}$ is the posterior median, see \autoref{table:estimated-params}. $\tau^*$ is the last NPI date (\autoref{table:NPI_dates}). Thin and bold lines show 95\% and 75\% credible intervals, respectively. \autoref{fig:tau-summary-mar28} shows a similar summary when estimating $\hat{\tau}$ using case data up to Mar~28, 2020, rather then Apr~11.
    }
...
We compare the official $\tau^*$ and effective $\hat{\tau}$ start of NPIs and find that in most regions the effective start of NPIs differs from the official date: in 10 of 12 countries the 75\% credible interval on $\hat{\tau}$ does not include $\tau^*$ (7 of 12 countries when considering a 95\% interval; \autoref{fig:tau-summary}).
...
In half of the examined regions, we estimate that the effective start of NPIs $\hat{\tau}$ is later than the official date $\tau^*$.
...
Interestingly, the effective start of of NPIs $\hat{\tau}$ in both France and Spain is estimated to have started on Mar~24, although the official NPI dates differ significantly: the first NPI in France is only one day before the last NPI in Spain.
...
In Sweden, the data do not "peak", i.e., the number of daily cases continues to grow up to Apr~11 (\autoref{fig:ppc}). 
...
We find one case in which the official and effective dates match "\emph{like a Swiss watch}" and the credible interval is narrow.
...
%%% Assessment
\paragraph*{\hl{Consequences of late and early effective start on assessment of NPIs impact.}}

The \emph{effective reproduction number} $R$ is the number of secondary
cases generated by an infectious case after an epidemic is already underway \citep{Bar-On2020}.
We infer the effective reproduction numbers before and after the implementation of NPIs from the estimated model parameters (\autoref{eq:Re}).
The impact of NPIs can then be measured by the relative reduction in the reproduction number \citep{Flaxman2020}. 

We compared the estimated NPIs impact with either the fixed~$\tau$ model or the free~$\tau$ model.
That is, the estimated impact assuming that NPIs started at the official date $\tau^*$ versus the estimate when inferring the effective start of the NPIs from the data.
\autoref{fig:Re} demonstrates that estimates from the fixed~$\tau$ model are consistently lower than estimates from a free~$\tau$ model, except for Sweden and Denmark, in which the inference uncertainty in is high.  
This suggests that if health officials and researchers assume that NPIs start at their official date, then their impact is likely to be under-estimated.
This might be interpreted as ineffectiveness of the NPIs, leading to further escalations.
...
This would lead to a significant underestimation, which might be interpreted as ineffectiveness of the NPI, leading to further escalations.
...
\begin{figure}[h]
    ...
    \caption{
    \textbf{\hl{Impact of NPIs is under-estimated when assuming they start at the official date.}}
    Shown are estimates of the relative reductions in $R$ (the effective reproduction number), which measures the impact of NPIs on disease transmission.
     The y-axis shows estimates when assuming the start of NPIs is fixed at the official date (fixed $\tau$); the x-axis shows estimates when inferring the effective start of NPIs from the data (free $\tau$). The dashed line shows a 1-to-1 correspondence.
     Markers and bars denote the posterior median and 50\% credible intervals.
    The relative reductions in $R$ are consistently lower for the fixed $\tau$ model (below the line), except for Sweden and Denmark in which uncertainty is high.
    } 
    \label{fig:Re}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{
    \textbf{\hl{Late effective start of NPIs leads to under-estimation of daily confirmed cases.}}
    ...
    Shaded box illustrates a generation interval of seven days.
...
In most investigated countries we find a late effective start of NPIs.
...
This difference might be explained, in some cases, by low compliance: In Italy, for example, the government plan to implement a lockdown in the Northern provinces leaked to the public, resulting in people leaving these provinces before the lockdown started~\citep{Gatto2020}.
...
\hl{As different countries experiment with various intervention strategies, we expect similar shifts to occur: in some cases the population will be late to comply with the guidelines, whereas in other cases the population will adopt restrictions or relaxations even before they are formally announced.}
Attempts to asses the impact of NPIs~\citep{Banholzer2020,Flaxman2020} \hl{generally assume they start at their official date, and that a significant change in the dynamics can be observed roughly seven-days after the start of NPIs} (due to the characteristic generation interval of \covid\citep{Bar-On2020}).
\hl{However, late and early effective start of NPIs, such as we have inferred, may bias these assessments and lead to wrong conclusions about the impact of NPIs} (\autoref{fig:Re}, \autoref{fig:prediction}).
...
\end{aligned}
\end{equation}
The relative reduction in the effective reproduction number due to NPIs is then $\frac{R_1 - R_2}{R1}$.
...
% generated with /python Table_estimated_params.py 2020-05-26-Apr11/
\begin{landscape}
\begin{table}[]
\centering
...
This work was supported in part by the Israel Science Foundation (3811/19, 552/19, and 1399/17).
}
...
% Table - WAIC 
% generated with /python Table-WAIC.py 2020-05-26-Apr11/
\begin{table}[h]
\centering
\caption*{\textbf{WAIC}}
\pgfplotstabletypeset[
...
% Table - RMSE 
% generated with /python Table-RMSE.py 2020-05-26-Apr11/
\begin{table}[h]
\centering
\caption*{\textbf{RMSE}}
\pgfplotstabletypeset[
...
\begin{figure}[b!]
    ...
    \textbf{Official vs. effective start of non-pharmaceutical interventions estimated up to Mar~28.}
    	The difference between $\hat{\tau}$ the effective and $\tau^*$ the official start of NPIs estimated from case data up to Mar~28, 2020, shown for different regions. Here, $\hat{\tau}$ is the posterior median, see \autoref{table:estimated-params}. $\tau^*$ is the last NPI date (\autoref{table:NPI_dates}). Thin and bold lines show 95\% and 75\% credible intervals, respectively.
    }
...
\begin{figure}
    ...
    Black lines represent a smoothing of the data points using a Savitzky-Golay filter. 
