...
Indeed, when examining the joint posterior distribution of $\tau$ and $\lambda$ (the latter is the reduction in transmission due to NPIs, see \autoref{eq:model}), high values of $\lambda$ are estimated in Denmark and Sweden (0.7 and 0.74, respectively; see~\autoref{table:estimated-params}), probably making the inference of $\tau$ more difficult due to unidentifiability, resulting in a wide posterior for $\tau$. Some correlation between the parameters is also evident in Norway, where $\lambda$ estimates are low (0.31).
...
In Wuhan, China, a lockdown was ordered on Jan 23~\citep{Li2020}, but we estimate the effective start of NPIs to be ten days later, at Feb~2 ($\substack{+2.29 \\ -2.97}$~days 95\% CI), although there is low but noticeable posterior probability for Jan~24-25 (\autoref{fig:late}). Higher estimates of $\tau$ (Jan~4) correspond to a wide posterior for $\lambda$ (\autoref{fig:joint}).
...
Indeed, Denmark and Sweden had the highest estimates of $\lambda$, that is, the smallest estimated effect of NPIs on transmission rates, which  probably hinders our ability to estimate $\tau$ in these countries (see also joint posterior distributions in \autoref{fig:joint}).
Moreover, in Sweden the number of daily cases continues to grow up to Apr~11, rather than "peak" (\autoref{fig:ppc}). 
...
There may also be an unidentifiability issue, as there is some negative correlation between the estimates of $\tau$ and $\lambda$ (\autoref{fig:joint}). 
...
The IAT was between 32K for Wuhan, China, and 187K for Germany~(\autoref{fig:autocorr}).
...
Therefore, 50 chains with 5M samples and IAT between 40K and 187K give an effective samples size between 1,335 and 7,812.
...
We also examined the joint posterior distributions of $\tau$ and $\lambda$ (\autoref{fig:joint}).
