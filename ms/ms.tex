...
However, the credible intervals are quite wide (\autoref{fig:tau-summary}), and the fit of the model with free $\tau$ wasn't better than the fit of the model with $\tau$ fixed at the official date (\autoref{table:WAIC}).
...
The estimated effective date in Norway is Mar~22, which is close to the official date of Mar~24.
However, the posterior distribution is very wide ($\pm$10.79 days 95\% CI; \autoref{fig:early}): it covers the range between Mar~9, three days before the first NPI, and Mar~29, five days  after the last NPI (see 
...
The success of non-pharmaceutical interventions is assessed by health officials using various metrics, such as the decline in the growth rate of daily cases. These assessments are made a specific number of days after the intervention began, to accommodate for the expected generation interval (i.e. average time between infection events in a chain of transmission), which is estimated at about 4-7 days~\citep{Bar-On2020}.

However, a significant difference between the beginning of the intervention and the effective change in transmission rates can invalidate assessments that assume a generation interval of 4-7 days and neglect the late or early population response to the NPI.
...
In most investigated countries we find a late effective start of NPIs on the transmission dynamics.
...
In contrast, in some regions we inferred reduced transmission rates even before official lockdowns were implemented (\autoref{fig:early}), although this is only conclusive in Germany (\autoref{fig:tau-summary}).
...
As expected, we have found that the evidence supports a model in which transmission rate changes at a specific time point $\tau$ over a model with a constant transmission rate (i.e. free or fixed model in \autoref{table:RMSE} and \autoref{table:WAIC}).
It may be interesting to check if the evidence favors a model with \emph{two} or more change-points, rather than one. 
Multiple such change-points could reflect escalating NPIs (e.g. school closures followed by lockdowns), or an intervention followed by a relaxation.
...
%%% Data %%%%
\paragraph*{Data.} 
We use daily confirmed case data $\vec{X}=(X_1, \ldots, X_T)$ from 12~regions during Jan--Apr 2020. These incidence data summarise the number of individuals $X_t$ tested positive for \sars (using RT-qPCR) at each day $t$.
...
This work was supported in part by the Israel Science Foundation 3811/19 and 1399/17.
}
