...
We hypothesized that the official and effective start date of such interventions can be noticeably different, for example due to slow adoption by the population, or because the authorities and the public are unprepared, and that these differences can lead to errors in the estimation of the impact of NPIs. \\
\textbf{Methods.}
SEIR models were fitted to case data from~12 regions to infer the effective start dates of interventions and contrast them with the official dates. The impact of NPIs was estimated from the inferred model parameters. \\
\textbf{Results.}
We infer mostly late effective start dates of interventions. For example, Italy implemented a nationwide lockdown on Mar~11, but we infer the effective start date on Mar~17 ($\substack{+3.05 \\ -2.01}$~days 95\%~CI).
In one case we infer an early effective start date: Germany announced a lockdown on Mar~22, but we infer an effective start date on Mar~19 ($\substack{+0.92 \\ -0.99}$~days 95\%~CI).
Moreover, we find that the impact of NPIs can be under-estimated if assuming they start at their official date.\\
\textbf{Conclusions.}
Differences between the official and effective start of NPIs are likely. Neglecting such differences can lead to under-estimation of the impact of NPIs, which could cause decision-makers to escalate interventions and guidelines. 
\\\\
{\small
\textbf{Keywords}: SEIR, COVID-19, public health, epidemic, infectious disease, NPI
}
\end{abstract}
...
We compare these estimates to the official dates, and find that they include mostly late effects of NPIs on infection dynamics.
...
Data for Wuhan, China, from Jan~10 to Feb~8, 2020, retrieved from \citet{Pei2020}. Data for 11 European countries, from Feb~20 to Apr~24, 2020, retrieved from \citet{Flaxman2020}.
...
\end{equation}
where $0 < \lambda < 1$.
That is, we assume that the transmission rate, $\beta$, the reporting rate, $\alpha$, and the confirmation rate, $p_t$, change after the implementation of NPIs, for example due to increased awareness and reduced contact rate in the population, and implementation of measures such as testing by the authorities.
...
Model fitting was calibrated for case data up to Mar~28, 2020, and then applied to data up to Apr~11, 2020 (for Wuhan, China, model fitting was applied for data up to Feb~8, 2020.)
...
We also tried to estimate the value of $p_t$ before and after $\tau$, instead of keeping it fixed at $1/9$ and $1/6$. The model with fixed values was supported by the evidence (lower WAIC, see~\autoref{eq:WAIC}) in 9 of 12 countries. Moreover, the estimates for Wuhan, China were $1/9$ and $1/6$, as in \citet{Li2020}. Hence, we fixed the values of $p_t$ to those used in \citet{Li2020} (see \autoref{eq:NPI_model}) in the analysis reported below.
...
Similarly, in France we estimate the effective start of NPIs at Mar~24, 2020 ($\substack{+2.65 \\ -1.44}$~days 95\%~CI, \autoref{fig:late}).
...
The only conclusive early case is Germany, in which we estimate the effective start of NPIs at Mar~19, 2020 ($\substack{+0.92 \\ -0.99}$~days 95~\%CI, \autoref{fig:early}).
...
The estimated effective date in Norway is Mar~22, 2020, two days earlier than the official date of Mar~24.
...
Switzerland ordered a national lockdown on Mar~20, 2020, after banning public evens and closing schools on Mar~13 and~14~\citep{Flaxman2020}.
...
\begin{figure}[h]
    ...
    The relative reductions in $R$ are lower for the fixed~$\tau$ model (below the dashed line) in at least seven of 12 regions.
    Note that the estimated effective start date is uncertain for Sweden, Denmark, and Norway (\autoref{fig:tau-summary}).
    The effect of the delay in effective start affects on the estimation error is shown in \autoref{fig:Re2}.
...
We find examples of mostly late but also early effective start of NPIs relative to the official date (\autoref{fig:tau-summary}).
...
Indeed, the evidence supports a change in infection dynamics (i.e. a model with fixed or free $\tau$) even for Sweden~(\autoref{table:RMSE}, \autoref{table:WAIC}, \autoref{fig:ppc}), where a lockdown was not implemented within the investigated time\footnote{Sweden banned public events on Mar~12, 2020, encouraged social distancing on Mar~16, and closed schools on Mar~18~\citep{Flaxman2020}.}.

Interestingly, the effective start of NPIs in France and Spain is estimated to have started on Mar~24 and~25, 2020, respectively,
...
It may be interesting to check if the evidence supports a model with \emph{two} or more change-points, rather than one, or even a continuous change.
...
However, both inference and interpretation of such models will be harder, as multiple change-points or continuous change are also likely to result in parameter unidentifiability, for example due to simultaneous implementation of NPIs~\citep{Flaxman2020}.

As different countries experiment with various intervention strategies, we expect similar shifts to continue to occur: in some cases the population will be late to comply with new guidelines, whereas in other cases the population will adopt restrictions, and possibly relaxations, even before they are formally announced.
...
We thank Yinon M. Bar-On, Lilach Hadany, Oren Kolodny, Zohar Yakhini, Tim CD Lucas, and two anonymous reviewers. 
} % small
...
\caption*{
	...
	In 7 of 11 cases, the model with free parameter $\tau$ has the lowest RMSE.
	...
	Bold values highlight the model with the lowest RMSE.
}
\label{table:RMSE}
\end{table}
...
\begin{figure*}[h]	
    ...
    ~
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\textwidth]{../figures/ppc/free/free_Austria_ppc_long.pdf}
    \end{subfigure}
    %%%
	\caption*{Belgium}    
    \begin{subfigure}{0.45\textwidth}
    ...
    ~
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\textwidth]{../figures/ppc/free/free_Belgium_ppc_long.pdf}
    \end{subfigure}
    %%%
	\caption*{Denmark}    
     \begin{subfigure}{0.45\textwidth}
    ...
    ~
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\textwidth]{../figures/ppc/free/free_Denmark_ppc_long.pdf}
    \end{subfigure}
	%%%
	\caption*{France}    
    \begin{subfigure}{0.45\textwidth}
    ...
    ~
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\textwidth]{../figures/ppc/free/free_France_ppc_long.pdf}
    \end{subfigure}
\end{figure*}
    %%%
\begin{figure*}
    ...
    ~
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\textwidth]{../figures/ppc/free/free_Germany_ppc_long.pdf}
    \end{subfigure}
    %%% 
	\caption*{Italy}    
    \begin{subfigure}{0.45\textwidth}
    ...
    ~
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\textwidth]{../figures/ppc/free/free_Italy_ppc_long.pdf}
    \end{subfigure}
    %%% 
	\caption*{Norway}    
    \begin{subfigure}{0.45\textwidth}
    ...
    ~
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\textwidth]{../figures/ppc/free/free_Norway_ppc_long.pdf}
    \end{subfigure}
    %%%  
	\caption*{Spain}     
    \begin{subfigure}{0.45\textwidth}
    ...
    ~
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\textwidth]{../figures/ppc/free/free_Spain_ppc_long.pdf}
    \end{subfigure}
\end{figure*}
    %%%
\begin{figure*}
    ...
    ~
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\textwidth]{../figures/ppc/free/free_Sweden_ppc_long.pdf}
    \end{subfigure}
    %%%  
	\caption*{Switzerland}    
    \begin{subfigure}{0.45\textwidth}
    ...
    ~
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\textwidth]{../figures/ppc/free/free_Switzerland_ppc_long.pdf}
    \end{subfigure}
    %%%  
    ...
    ~
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\textwidth]{../figures/ppc/free/free_United_Kingdom_ppc_long.pdf}
    \end{subfigure}
    \caption{    
    ...
    Coloured lines represent posterior predictions from a model with fixed~$\tau$ (blue) and free~$\tau$ (green). Models were fitted with data up to Apr~11. The predictions are generated by drawing 1,000 parameters sets from the posterior distribution, and then generating a daily case count using the SEIR model (\autoref{eq:model}) up to Apr~24, 2020. Note the differences in the y-axis scale.
...
\begin{figure}[h]
	...
	The dashed gray and black lines show a linear regression with and without SE, DK and CN, respectively
	(with: slope=$-0.019\pm0.026$~95\%~CI, intercept=$-0.105 \pm0.175$~95\%~CI, $R^2=0.211$;
     without: slope=$-0.016 \pm0.006$~95\%~CI, intercept=$-0.001 \pm0.036$~95\%~CI, $R^2=0.845$.)
     Note that the estimated effective start date is uncertain for Sweden, Denmark, and Norway (\autoref{fig:tau-summary}).
	AT: Austria, BE: Belgium, CH: Switzerland, CN: Wuhan, China, DE: Germany, DK: Denmark, ES: Spain, FR: France, GB: United Kingdom, IT: Italy, NO: Norway, SE: Sweden.
    } 
    \label{fig:Re2}
...
\begin{figure}[h]
    ...
    Vertical line shows Mar~8, 2020, the effective start of NPIs $\hat{\tau}$ in both countries.
