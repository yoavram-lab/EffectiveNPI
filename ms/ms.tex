...
Keywords: SEIR, COVID-19, public health, pandemic, infectious disease, NPI

%Classification: Biological Sciences / Population Biology; Social Sciences / Environmental Sciences
...
We infer the effective reproduction numbers before and after the implementation of NPIs from model parameters (\autoref{eq:Re}).
...
These results suggest that the impact of past NPIs by health officials and researchers is likely to be under-estimated if they assume NPIs effectively start at their official dates.
...
This would lead to a significant under-estimation, which might be interpreted as ineffectiveness of the NPI, leading to further escalations.
...
\begin{figure}[h]
    ...
    \textbf{(A)} Using the official date $\tau^*$ for the effective start of the NPI, the model under-estimates the number of cases seven days after the start of the NPI.
    \textbf{(B)} Using the estimated date $\hat{\tau}$ for the effective start of the NPI, the model precisely estimates the number of cases seven days after the start of the NPI.
...
Attempts to asses the impact of NPIs~\citep{Banholzer2020,Flaxman2020} generally assume they start at their official date, and that a significant change in the dynamics can be observed roughly seven days after the start of NPIs (due to the characteristic generation interval of \covid\citep{Bar-On2020}).
...
%TODO Convergence and merge to previous paragraph
The integrated autocorrelation time (REF) was averaged across all parameters and chains, and found to be between VALUE FOR NORWAY and VALUE FOR GERMANY (Fig. SX). This means that in our 50 chains, with 7M samples per chain and 2M burnin samples, contain between XXX and XXX effectively uncorrelated samples (PLEASE CALCULATE THESE XXX and XXX numbers).
We examined the trace plots for TAU in all regions (Fig. SX). All chains seem to converge to the same stationary distribution, in most cases before 2M steps (thus our choice of burnin time). In the case of Spain, single chain converged at around X steps (VERIFY!!). This single chain was considered part of the burnin and removed in the analysis. We also ran additional chains, which we initialized with a different seed and ran for 2M steps. The estimated posterior distributions of TAU are very similar to our main analysis (Fig. SX), further increasing our confidence in the convergence of our inference.
