...
However, behavioural and social change does not occur immediately, but rather requires time to diffuse in the population through media, social networks, social interactions, and even cognitive processes. % REF for delay in behavioural adaptation
% UO: If you want more text. you can add that social behaviour and compliance/adopting public health recommendations is very important in infectious diseases and perhaps cite/say a word about vaccines, HIV and condoms or something like that
...
Here, we hypothesise that there is a significant difference between the official start of NPIs and the start of their adoption by the public.
...
% UO: Perhaps add several sentences on how important such a delay is on estimating the effect of interventions and even planning them (which should perhaps be done while considering the expected delay)?
...
In regions in which there were multiple sequences of days with zero confirmed cases (e.g. France), we cropped the data to begin with the last sequence so that our analysis focuses on the first sustained outbreak rather than isolated imported cases. 
...
Here we estimate the posterior distribution of \emph{effective} start date of the NPI, $P(\tau \mid \vec{X}$, as well as maximum a posteriori (MAP) estimates, $\hat{\tau}$, by jointly estimating $\tau, \beta, \lambda, \alpha_1, \alpha_2$ on the entire time series per region (e.g. Italy, Austria), rather than splitting the region time series at $\tau^*$. 
In all examined cases considering NPIs leads to a better model fit: the DIC of a model without NPI ($\beta_t \equiv \beta, \alpha_t \equiv \alpha, p_t \equiv p$ for all $t$) was higher than the DIC of a model with NPI (\autoref{eq:NPI_model}) by at least \hl{Z}.
\autoref{fig:fig1} compares the official dates $\tau^*$ and our MAP estimates $\hat{\tau}$, with confidence intervals. 
% OU: I would report a) that we use the models with NPI because delta DIC>X, and b) 95% CIs of (tau*-tau^). E.g. if your CI on tau^ is (10,14) and tau* is 7, you have a CI of (3,7), which is easily interpreted as the confidence of the delay in the effective implementation of the CI.
