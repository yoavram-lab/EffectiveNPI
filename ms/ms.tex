...
Furthermore, compliance to NPIs may be impacted by risk perception, as percieved by the number of domestic cases or even by reported cases in other regions and countries.
...
\begin{figure}[h]
    ...
    \caption{Official and effective start of non-pharmaceutical interventions.}
    \label{fig:fig1}
\end{figure}
%%%%%%%%%%%%%%
...
These studies have assumed that the parameters of the epidemiological model change at a specific date, as in \autoref{eq:NPI_model}, and set the change date $\tau$ to the official NPI date $\tau^*$. They then fit the model once for time $t<\tau^*$ and once for time $t \ge \tau^*$ (see \hl{TABLE2} for a summary of official NPI dates.)
...
We then compute the maximum a posteriori estimate $\hat{\tau}=\mathit{argmax}_{\tau}{P(\tau \mid \vec{X})}$.
...
We compare the official $\tau^*$ and effective $\hat{\tau}$ start of NPIs and find that in most regions the effective start of NPI  significantly differs from the official date (\autoref{fig:fig1}): the \hl{75\%} confidence interval on $\hat{\tau}$ does not include $\tau^*$, and the DIC of the model with free $\tau$ parameter is lower than that of a model with a fixed $\tau \equiv \tau^*$ (\hl{$\Delta DIC > ?$}.) The exception that proves the rule is \hl{Switzerland}.
...
\begin{figure}[h]
    ...
    \caption{Delayed effect of non-pharmaceutical interventions in Italy and Wuhan, China.}
    \label{fig:fig2}
\end{figure}
%%%%%%%%%%%%%%
...
However, we estimate the effective date $\hat{\tau}$ at Mar~16 (the posterior probability that $\tau$ is later than Mar~11 is \hl{($P(\tau > \tau^*)=???$)} (\autoref{fig:fig2}).
Similarly, in Wuhan, China, lockdown was declared on Jan 23~\citep{Li2020}, but we estimate  the effective start of NPIs to be 3-4 days layer \hl{($P(\tau > \tau^*)=???$)} (\autoref{fig:fig2}).

%%% Fig 3 %%%
\begin{figure}[h]
    \centering
	\includegraphics[width=\textwidth]{../figures/Fig3.pdf}
    \caption{Advanced effect of non-pharmaceutical interventions in Spain and France.}
    \label{fig:fig3}
\end{figure}
%%%%%%%%%%%%%%
...
Nevertheless, we estimate the effective start of NPI $\hat{\tau}$ at Mar~8 or~9, rather than Mar 14 \hl{($P(\tau < \tau^*)=???$)} (\autoref{fig:fig3}).
...
However, we hypothesize that a significant difference between the beginning of the intervention and the effective change in transmission rates invalidates assessments that assume 4-7 days.
...
\begin{figure}[h]
    ...
    \caption{Delayed effective start of NPI causes leads to under-estimation of daily confirmed cases. The red and black lines show model predictions when NPIs start on the official date $\tau^*$ or on the effective date $\hat{\tau}$, respectively, with 95\% confidence intervals. This demonstrates $\Delta$ the assessment error seven days after the official start of NPIs, which in this case is about 40\%.
    Parameters are MAP estimates for Italy (\hl{TABLE}).
    } 
    \label{fig:fig4}
\end{figure}
%%%%%%%%%%%%%%
...
In contrast, in most investigated countries, such as \hl{Spain and France}, transmission rates seem to have been reduced even before official lockdowns were implemented (\autoref{fig:fig3}).
...
\hl{check if this is true} Indeed, the evidence supports a change in transmission dynamics (i.e. a model with $\tau$) even for Sweden, in which a lockdown was not implemented~\citep{Flaxman2020}, suggesting that lockdowns may not be necessary if other NPIs are adopted early enough during the outbreak (Sweden banned public events on Mar~12, encouraged social distancing on Mar~16, and closed schools on Mar~18~\citep{Flaxman2020}.)
...
Such delays and advances could confuse analyses and lead to wrong conclusions about the effects of NPI removals. % TODO figure for assessment
